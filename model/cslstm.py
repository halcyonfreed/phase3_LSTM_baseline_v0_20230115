...
def outputActivation(x): #x是future_pred 三维tensor
    ...
    sigX = torch.exp(sigX)
    sigY = torch.exp(sigY)
    rho = torch.tanh(rho) #有点像二元正态分布，ux,uy,σx,σy,ρ相关系数
...
class csLSTM(nn.Module):    
    ...
    def __init__(self,args):
        ...
        self.soc_embedding_size = (((args['grid_size'][0]-4)+1)//2)*self.conv_3x1_depth #？13-4=9 10//2=5 为什么//2


        # 定义network layer的weight
        self.ip_emb = torch.nn.Linear(2,self.input_embedding_size)# 输出[2,input_embedding_size], Input embedding layer: embedding用的是全连接层
